...
tf.app.flags.DEFINE_string('VOC_dir', '../dataset/OCR/RafI_Latest_OCR/train/', 'Voc dir')
...
tf.app.flags.DEFINE_string('dataset', 'OCR', 'dataset')
            ...
            for child_item in child_of_root:
                ...
                if child_item.tag == 'robndbox':
                    ...
                    for node in child_item:
                        orig_tmp_box.append(float(node.text))
                        print(orig_tmp_box)
                    tmp_box = rotate(orig_tmp_box)
                    print(tmp_box)
...
def rotate(orig_tmp_box):
    a = tuple(orig_tmp_box) # tuple creation to get the center
    center_tuple = (a[0],a[1])
    center = list(center_tuple)
    width = orig_tmp_box[2]
    height = orig_tmp_box[3]
    angle = orig_tmp_box[4]
